...
class HorseSatisfyConstraintSuperInteraction(SatisfyConstraintSuperInteraction):
    INSTANCE_SUBCLASSES_ONLY = False

    def should_link_carried_sims(self):
        return True

    def get_require_reins_up(self, slave_data:'RoutingFormation') -> 'bool':
        return True

    def __init__(self, *args, original_interaction:'Interaction'=None, retry_count=3, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _exited_pipeline(self, *args, **kwargs):
        ...
        if self.is_finishing:
            if self.finishing_type != FinishingType.INTERACTION_QUEUE or self.retry_count <= 0:
                self.original_interaction.is_waiting_paired_horse_carry = PairedHorseTransitionState.FINISHED
            else:
                dest_result = self.sim.push_super_affordance(self.affordance, self.target, self.context, constraint_to_satisfy=self._constraint_to_satisfy, set_work_timestamp=False, original_interaction=self.original_interaction, retry_count=self.retry_count - 1, name_override='RouteRiderClose')
                if dest_result:
                    self.original_interaction.is_waiting_paired_horse_carry = PairedHorseTransitionState.WAITING
                else:
                    self.original_interaction.is_waiting_paired_horse_carry = PairedHorseTransitionState.FINISHED
        else:
            self.original_interaction.cancel(finishing_type=FinishingType.USER_CANCEL, cancel_reason_msg='Canceled Ride Nearby.')
